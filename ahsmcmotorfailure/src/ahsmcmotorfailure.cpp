...
public:
    HSMCUAV(): Node("hsmc_uav"), prephir(0), prethetar(0), prepsir(0), controlxy_(0.05, 0.1, 0.05, 0.1)
    {
        // Publisher for propeller velocities
        propvel_pub_ = this->create_publisher<std_msgs::msg::Float64MultiArray>("/prop_vel", 10);
        state_sub_ = this->create_subscription<nav_msgs::msg::Odometry>("/droneposition/odom", 10, std::bind(&HSMCUAV::stateCallback, this, std::placeholders::_1));
        last_time_ = this->now();
    }
    void stateCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
        ...
        double xr = 5.0, yr = 5.0, zr = 5.0, psir = 0;
        ...
        RCLCPP_INFO(this->get_logger(), "Phir, thetar: %f, %f", phir, thetar);
        ...
        double cphi_ctrl = 1.05;
        double ctheta_ctrl = 1.05;
        ...
        double U2 = clamp(U2s, -2.0, 2.0);
        double U3 = clamp(U3s, -2.0, 2.0);
        ...
        //std::ofstream outfile1("datafailure/datafailurexyz.txt", std::ios_base::app);
        //if (outfile1.is_open()) {
        ...
        //std::ofstream outfile2("datafailure/datafailurerpy.txt", std::ios_base::app);
        //if (outfile2.is_open()) {
        //    outfile2 << "Time: " << elapsed_time_ms << ", Roll desi: " << phir << ", Pitch desi: " << thetar << ", Yaw desi: " << psir << ", Roll: " << phi << ", Pitch: " << theta << ", Yaw: " << psi << ", Rolld: " << phid << ", Pitchd: " << thetad << ", Yawd: " << psid << "\n";
        //    outfile2.close();
        //} else {
        //    RCLCPP_ERROR(node->get_logger(), "Unable to open file for writing.");
        //}

        //std::ofstream outfile3("datafailure/datafailurecontrol.txt", std::ios_base::app);
        //if (outfile3.is_open()) {
